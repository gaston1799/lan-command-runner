...
async function runJob(job, context) {
  ...
  const result = String(job.type || "").startsWith("file.")
    ? await runFileJob(job, postProgress)
    : await runCommand(job, postOutput);
...
async function agent(options) {
  ...
  while (!agentId || !agentToken) {
    try {
      await register();
    } catch (error) {
      if (!registrationLogged) {
        console.error(`[lcr] Waiting for broker at ${brokerUrl}`);
  ...
  while (true) {
    try {
      ...
      const result = await runJob(poll.job, { brokerUrl, enrollToken, name, agentId, agentToken });
