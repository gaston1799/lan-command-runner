...
$installRoot = if ($env:LCR_INSTALL_ROOT) { $env:LCR_INSTALL_ROOT } else { Join-Path $env:LOCALAPPDATA 'lan-command-runner' }
$cli = Join-Path $installRoot 'bin\\lcr.js'
if (-not (Test-Path -LiteralPath $cli)) { throw "Missing LCR CLI at $cli" }
$stdoutLog = Join-Path $env:TEMP 'lcr-agent-stdout.log'
$stderrLog = Join-Path $env:TEMP 'lcr-agent-stderr.log'
$agentArgs = @($cli, 'agent', '--url', ${psString(brokerUrl)}, '--token', ${psString(enrollToken)}, '--name', ${psString(name)}, '--id', ${psString(agentId)})
Start-Process -FilePath $node -ArgumentList $agentArgs -WindowStyle Hidden -RedirectStandardOutput $stdoutLog -RedirectStandardError $stderrLog
"[$(Get-Date -Format o)] Restarted agent using $node $cli" | Add-Content -LiteralPath $logPath
